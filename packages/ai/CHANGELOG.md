...
- Fixed generated model catalog publication to fsync complete staged trees and replace catalog directories as a unit, restoring the previous catalog when publication fails after the target is moved.
- Fixed generated model catalog publication on Windows by opening staged files with write access before `fsync`, preserving durability checks without triggering `EPERM`.
