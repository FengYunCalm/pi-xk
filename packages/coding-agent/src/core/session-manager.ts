...
export const COMPACTION_RECOVERY_PROMPT_VERSION = "compaction-recovery-v1" as const;
export type CompactionRecoveryPromptVersion = typeof COMPACTION_RECOVERY_PROMPT_VERSION;

export interface CompactionEntry<T = unknown> extends SessionEntryBase {
	type: "compaction";
	summary: string;
	/** Short historical label. Legacy entries may omit it. */
	title?: string;
...
/**
 * Return the latest recoverable compaction until a non-error, non-aborted
 * assistant response proves that a subsequent logical run completed.
 */
export function getPendingCompactionRecovery(entries: SessionEntry[]): CompactionEntry | null {
	let compactionIndex = -1;
	for (let i = entries.length - 1; i >= 0; i--) {
		const entry = entries[i];
		if (entry.type === "compaction" && entry.recoveryPromptVersion === COMPACTION_RECOVERY_PROMPT_VERSION) {
			compactionIndex = i;
			break;
		}
	}
	...
	return entries[compactionIndex] as CompactionEntry;
}
...
export class SessionManager {
	...
	appendCompaction<T = unknown>(
		...
		details?: T,
		fromHook?: boolean,
		metadata?: Pick<CompactionEntry, "title" | "reason" | "recoveryPromptVersion">,
	): string {
		const entry: CompactionEntry<T> = {
			...
			recoveryPromptVersion: metadata?.recoveryPromptVersion,
			details,
			fromHook,
		};
		this._appendEntry(entry);
