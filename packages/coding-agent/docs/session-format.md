...
interface CompactionSummaryMessage {
  ...
  summary: string;
  title?: string;              // Historical UI label; not included in LLM summary text
  tokensBefore: number;
  timestamp: number;
}
```
...
Created when context is compacted. Stores a summary of earlier messages plus optional discovery and recovery metadata. Legacy entries may omit all new optional fields.

```json
{"type":"compaction","id":"f6g7h8i9","parentId":"e5f6g7h8","timestamp":"2024-12-03T14:10:00.000Z","title":"Authentication refactor","summary":"User discussed X, Y, Z...","firstKeptEntryId":"c3d4e5f6","tokensBefore":50000,"reason":"threshold","recoveryPromptVersion":"compaction-recovery-v1"}
```

Optional fields:
- `title`: Validated single-line historical label. Native titles are at most 60 Unicode code points; legacy entries may omit it.
- `reason`: `"manual"`, `"threshold"`, or `"overflow"`.
- `recoveryPromptVersion`: Currently `"compaction-recovery-v1"`. It marks recovery context as pending until a later non-error, non-aborted assistant response.
- `details`: Implementation-specific data (e.g., `{ readFiles: string[], modifiedFiles: string[] }` for default, or custom data for extensions)
- `fromHook`: `true` if generated by an extension, `false`/`undefined` if pi-generated (legacy field name)

The title is available to the interactive renderer but `convertToLlm()` sends only the summary body. Recovery metadata does not create a transcript message; `AgentSession` uses it to append one-time system context to the next actual logical run.
...
2. Converts selected entries to messages:
   ...
   - `compaction` -> `compactionSummary` (title retained for UI; only summary body is projected to the LLM)
...
- `appendCompaction(summary, firstKeptEntryId, tokensBefore, details?, fromHook?, metadata?)` - Add compaction; metadata may contain `title`, `reason`, and `recoveryPromptVersion`
