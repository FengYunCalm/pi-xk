...
import { GOAL_FILE_SCHEMA, type GoalContractV1 } from "./contract.ts";
...
export function goalIdentityFingerprint(contract: GoalContractV1): string {
...
function headerFor(contract: GoalContractV1, kind: GoalFileKind): GoalFileHeader {
	return {
		schema: GOAL_FILE_SCHEMA,
		kind,
		goalId: contract.goalId,
...
function renderObjective(contract: GoalContractV1): string {
	return [
		...
		"Call pi_xk_end_goal only after you have updated goal-state.md and verified the objective and declared acceptance evidence are complete.",
		...
		...(contract.acceptance.length > 0
			? contract.acceptance.map((item) => `- ${item.id}: ${item.description}`)
			: ["- None declared."]),
		"",
	].join("\n");
}

function renderState(contract: GoalContractV1): string {
	return [
...
export async function createGoalFiles(goalDirectory: string, contract: GoalContractV1): Promise<void> {
...
export async function inspectGoalFiles(goalDirectory: string, contract: GoalContractV1): Promise<GoalFilesDiagnostic> {
	const paths = goalFilePaths(goalDirectory);
	return {
		goalId: contract.goalId,
		fingerprint: goalIdentityFingerprint(contract),
